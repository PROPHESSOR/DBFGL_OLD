(function () {
	...
	function modlistService ($q, $rootScope, nwService) {
		 ...
		 * @type {String}
		 */
		const listDir = $rootScope.config.modlistpath;
		...
		service.rename = function (item) {
			...
			const newPath = nwService.buildPath([listDir, item.name + '.json']);
		...
		service.saveSelected = function (listObj) {
			const existingIndex = _.findIndex(service.lists, function (item) {
				return listObj.name === item.name;
			});

			const file = nwService.buildPath([listDir, listObj.name + '.json']);
		...
		service.getLists = function () {
			...
			nwService.getDir(listDir).then(function (items) {
				service.lists = items.map(function (item) {
					const file = nwService.buildPath([listDir, item]);
